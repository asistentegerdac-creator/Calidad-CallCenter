  ...
  return (
    <div className="glass-card bg-white p-12 border border-orange-100 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center gap-5 mb-10">
        ...
        <div>
          <h3 className="text-2xl font-black text-slate-900 tracking-tight">Nueva Auditoría Médica</h3>
          <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mt-1">Ingreso de incidencia para control de calidad</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="space-y-2 lg:col-span-2">
            ...
            <input required className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl p-4 font-bold text-slate-900 outline-none focus:border-amber-400" value={formData.patientName} onChange={e => setFormData({...formData, patientName: e.target.value})} placeholder="Pérez López, Juan" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Teléfono <span className="text-rose-500">*</span></label>
            <input required type="tel" className="w-full bg-white border-2 border-amber-200 rounded-xl p-4 font-bold text-slate-900 outline-none focus:border-amber-400 shadow-sm" value={formData.patientPhone} onChange={e => setFormData({...formData, patientPhone: e.target.value})} placeholder="Ej. 123-456-7890" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nivel de Satisfacción Percibido</label>
          ...
          <div className="space-y-2">
             ...
             <div className="flex bg-slate-50 rounded-xl p-2 gap-2">
                {[ComplaintStatus.PENDIENTE, ComplaintStatus.PROCESO].map(s => (
                  <button type="button" key={s} onClick={() => setFormData({...formData, status: s})} className={`flex-1 py-3 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${formData.status === s ? 'bg-slate-900 text-white shadow-md' : 'text-slate-400'}`}>{s}</button>
                ))}
             </div>
          </div>
        </div>
        ...
        <button disabled={loading} className="w-full py-6 neo-warm-button rounded-[2.5rem] font-black text-[11px] uppercase tracking-[0.4em] shadow-2xl disabled:opacity-50">
          {loading ? 'ANALIZANDO CON IA DE CALIDAD...' : 'REGISTRAR AUDITORÍA OFICIAL'}
        </button>
      </form>
    </div>
  );
