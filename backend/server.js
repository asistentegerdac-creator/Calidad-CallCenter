...
const syncSchema = async (targetPool) => {
  ...
  try {
    ...
    console.log("✅ [DAC] Esquema Postgres sincronizado.");
  } catch (err) {
    await client.query('ROLLBACK');
    console.error("❌ [DAC] Error esquema:", err.message);
  } finally {
    client.release();
  }
};
...
const loadConfig = async () => {
  if (fs.existsSync(CONFIG_PATH)) {
    try {
      ...
      console.log(`🔗 [DAC] Nodo Activo: ${config.database}`);
    } catch (e) { console.error("❌ [DAC] Fallo carga config inicial"); }
  }
};
loadConfig();
  ...
  try {
    await pool.query(`
      ...
      ON CONFLICT (audit_id) DO UPDATE SET 
        incidence_date=EXCLUDED.incidence_date, patient_name=EXCLUDED.patient_name, current_status=EXCLUDED.current_status, management_solution=EXCLUDED.management_solution, resolved_by_admin=EXCLUDED.resolved_by_admin
...
app.post('/api/areas-config', async (req, res) => {
  ...
  try {
    ...
    // ACTUALIZACIÓN SELECTIVA: Solo pendientes y en proceso
...
const PORT = 3008;
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 [DAC] Backend v8.0 en puerto ${PORT}`));
