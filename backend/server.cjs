...
app.get('/api/complaints', async (req, res) => {
  ...
  try {
    ...
    `, [c.id, c.date, c.patientName, c.patientPhone, c.doctorName, c.specialty, c.area, c.managerName, c.description, c.status, c.priority, c.satisfaction, c.managementResponse, c.resolvedBy, c.resolvedAt, c.registeredAt, c.isObserved || false, JSON.stringify(c.responseHistory || []), c.dimension, JSON.stringify(c.evidenceImages || []), c.involvedPersonnel, c.actionTaken, c.correctiveMeasure, c.correctiveMeasureOther]);
    res.sendStatus(201);
  } catch (e) { res.status(500).send(e.message); }
});
