...
const App: React.FC = () => {
  ...
  const [activeCall, setActiveCall] = useState<IPCall | null>(null);
  ...
  const handlePhoneAction = (action: 'answer' | 'hangup' | 'dial', number?: string) => {
    if (action === 'answer' && activeCall) {
      ...
      if (activeCall) {
        const now = Date.now();
        const duration = callStartTimeRef.current ? Math.floor((now - callStartTimeRef.current) / 1000) : 0;
        const finalStatus = (activeCall.direction === 'incoming' && activeCall.status === 'ringing') ? 'missed' : 'completed';
  ...
  // Simulación de llamadas entrantes cuando está Online
  useEffect(() => {
    let timer: any;
    if (phoneConfig.status === 'online' && !activeCall) {
      timer = setInterval(() => {
        if (Math.random() > 0.95) { // 5% de probabilidad cada 5 seg
          const incoming: IPCall = {
          ...
          setPhoneConfig(prev => ({ ...prev, status: 'ringing' as any }));
        }
      }, 5000);
    }
            ...
            <div className="p-12">
              {isRegistering ? (
                <form onSubmit={handleRegister} className="space-y-6">
                  <div className="text-center mb-4"><h2 className="text-2xl font-black text-white">Nuevo Operador</h2></div>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                  <div className="text-center mb-4"><h2 className="text-2xl font-black text-white">Acceso DAC</h2></div>
                  <div className="space-y-4">
                    <input name="user" type="text" required placeholder="Usuario" className="w-full bg-slate-900/40 border border-slate-700 rounded-2xl px-6 py-4 focus:border-blue-500 outline-none transition-all text-white" />
                    <input name="pass" type="password" required placeholder="Clave" className="w-full bg-slate-900/40 border border-slate-700 rounded-2xl px-6 py-4 focus:border-blue-500 outline-none transition-all text-white" />
                  </div>
    ...
    <div className="min-h-screen flex bg-[#f8fafc] font-['Inter']">
      <aside className="hidden lg:flex w-72 bg-white border-r border-slate-200 flex-col fixed inset-y-0 shadow-xl z-30">
        <div className="p-10 mb-4">
           <div className="flex items-center gap-3">
             ...
             <div><h1 className="text-xl font-black text-slate-900 tracking-tighter">Calidad DAC</h1><p className="text-[9px] text-blue-500 font-bold uppercase tracking-widest mt-1">OPERACIONES</p></div>
           </div>
        </div>
        ...
        <div className="p-6">
           <div className="bg-slate-900 rounded-[30px] p-6 text-white shadow-2xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="overflow-hidden"><p className="text-xs font-black truncate">{currentUser?.name}</p><p className="text-[9px] text-blue-400 font-bold uppercase tracking-widest">Master Op</p></div>
              </div>
      ...
      <main className="flex-1 lg:ml-72 p-6 md:p-10 lg:p-12 pb-32 min-h-screen">
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
          <div><p className="text-blue-600 font-black uppercase text-[10px] tracking-[0.3em] mb-2">Central DAC v4.0</p><h2 className="text-4xl font-black text-slate-900 tracking-tighter capitalize">{activeView.replace('-', ' ')}</h2></div>
          <div className="flex items-center gap-6 bg-white px-6 py-3 rounded-full border border-slate-100 shadow-lg">
             <div className="text-right"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Grandstream IP</p><p className={`text-xs font-bold flex items-center gap-1 justify-end ${phoneConfig.status === 'offline' ? 'text-slate-400' : 'text-green-500'}`}><span className={`w-2 h-2 rounded-full ${phoneConfig.status === 'offline' ? 'bg-slate-300' : 'bg-green-500 animate-pulse'}`}></span>{phoneConfig.status.toUpperCase()}</p></div>
             <div className="w-12 h-12 bg-blue-50 rounded-full flex items-center justify-center text-blue-600 relative"><span className="text-xl">📡</span>{phoneConfig.status !== 'offline' && <span className="absolute top-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>}</div>
          </div>
        </header>
        <div className="animate-in fade-in duration-700">{renderView()}</div>
      </main>

      <PhoneWidget config={phoneConfig} activeCall={activeCall} onAction={handlePhoneAction} />
...
const NavButton = ({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: string, label: string }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black transition-all ${active ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-50 hover:text-blue-600'}`}><span className="text-xl">{icon}</span><span className="tracking-tight text-xs uppercase font-black">{label}</span></button>
);

const MobileNavButton = ({ active, onClick, icon }: { active: boolean, onClick: () => void, icon: string }) => (
  <button onClick={onClick} className={`w-12 h-12 rounded-2xl transition-all flex items-center justify-center ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400'}`}><span className="text-xl">{icon}</span></button>
);
